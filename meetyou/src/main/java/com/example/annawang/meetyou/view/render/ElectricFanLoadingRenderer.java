package com.example.annawang.meetyou.view.render;
...
import com.example.annawang.meetyou.util.DensityUtil;
import com.example.annawang.meetyou.view.LoadingRenderer;
...
public class ElectricFanLoadingRenderer extends LoadingRenderer {
    ...
    public static final long ANIMATION_DURATION = SlideSample.DURATION_SLIDE * 15;
    ...
    private static final String TOTAL_TEXT = "需要一口袋糖果般的吻";

    private static final String PERCENTAGE_100 = "(*￣3￣*)";
    ...
    private void init(Context context) {
        ...
        mRepeatCount = ANIMATION_REPEAT_COUNT;
    ...
    protected void draw(Canvas canvas, Rect bounds) {
        ...
        if (mScale < 1.0f) {
            ...
            mPaint.setTextSize(mTextSize * (1 - mScale));
            mPaint.setColor(mElectricFanOutlineColor);
            Rect textRectShow = new Rect();
            mPaint.getTextBounds(TOTAL_TEXT, 0, TOTAL_TEXT.length(), textRectShow);
            canvas.drawText(TOTAL_TEXT, arcBounds.centerX() - textRectShow.width() / 2.0f,
                    arcBounds.centerY() / 2, mPaint);
        }
